...
public:
    Receiver (const std::string &topicColor, 
    ...
    void run () {
        pub = nh.advertise<pcl::PointCloud<pcl::PointXYZRGBA>> ("kinect2_pointcloud", 1);
        start();
        stop();
    }
...
private:
    ...
    void cloudFilter () {
        ...
        while (ros::ok()) {
            ...
            ROS_INFO("SIZE : %d", (int)out_cloud.points.size());
...
#endif
    ros::init(argc, argv, "kinect2_pointcloud_rgb");
