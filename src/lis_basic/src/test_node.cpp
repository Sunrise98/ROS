...
public:
    Circle_path() {
        ...
        //決め打ちの右手の姿勢
        hand_poses.right.position.x    = 0.575;
        hand_poses.right.position.y    = -0.183;
        hand_poses.right.position.z    = 0.257;
        hand_poses.right.orientation.x = 0.136;
        hand_poses.right.orientation.y = 0.990;
        hand_poses.right.orientation.z = -0.010;
        hand_poses.right.orientation.w = 0.023;
    ...
    void gen_path() {
        ...
        ros::Rate loop_rate(10);//0.3Hzでfor文を回してデータをpublishする

        for (int i = 0; i < 3; i++) {
            //現在時刻をros::Time型の構造体に保存
            ros::Time current_time = ros::Time::now();

            //desk座標系でみた目標左手先位置の設定

            //tf::broadcast書き方１　一度tf::Transform型に目標姿勢を入力してからsendTransformする場合
            //~ transform.setRotation(tf::Quaternion(180.0*M_PI/180.0, 0.0, 0.0*M_PI/180.0));//回転軸は,z,y,xの順
            //~ transform.setOrigin( tf::Vector3(0.3, 0.3+0.1*i, 0.1) );//desk座標系から見た目標手先位置の設定(m単位)
            //~ br.sendTransform(tf::StampedTransform(transform, current_time, "/desk", "/left_target"));//tfに目標左手先/left_targetを登録

            //~ //tf::broadcast書き方2 まとめて１行で書いてしまう場合
            br.sendTransform(tf::StampedTransform(tf::Transform(tf::Quaternion(0.0*M_PI/180.0, 0.0*M_PI/180.0, 0.0*M_PI/180.0), tf::Vector3(0.0, 0.0+0.05*i, -0.2)), current_time, "/desk", "/left_target"));
